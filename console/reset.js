...
const { projectService, contractorService } = require('../services');

const { Contractor, Project } = dbService.models;
...
const createContractors = (data) =>
  Promise.all(data.map(contractorService.create));

const createProject = async ({ contractors: contractorsData, ...data }) => {
